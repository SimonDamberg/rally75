-- Seed: the whole stable, copied from ALL_KUSKAR (src/shared/content/kuskar.ts).
-- src/shared/content/kuskSeed.test.ts checks that the newest seed file stays in sync.
--
-- Replaces the eight originals from 20260914000004_kusk_seed.sql (their notes were rewritten) and
-- adds the guest cameos. Deleting by name rather than truncating leaves any kusk Simon added by
-- hand in the GM app untouched.

delete from public.kusks where name in ('Jesper', 'GP', 'Emma', 'Axel', 'Erik', 'Kajsa', 'Simon', 'Palm');
...
insert into public.kusks (name, title, notes) values
  ...
  ('Travpensionären Bengt', 'nästa vecka tar han igen det', array[
