...
export function Spotlight({ race, bets }: { race: RaceRow; bets: readonly BetRow[] | undefined }) {
  ...
  const from = race.betting_at ? Date.parse(race.betting_at) : Date.parse(race.created_at)
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-col gap-[1.5vh] overflow-hidden">
      ...
      <div className="flex shrink-0 items-center gap-3">
        {race.field.map((h, j) => (
          <span key={h.n} className="relative h-2 flex-1 overflow-hidden rounded-full bg-white/15" aria-hidden>
            <span
              className="absolute inset-y-0 left-0 rounded-full bg-plate"
              style={{ width: j < i ? '100%' : j === i ? `${Math.round(progress * 100)}%` : '0%' }}
            />
          </span>
        ))}
      </div>
