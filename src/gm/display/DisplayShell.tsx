...
export function DisplayShell() {
  ...
  const settledId = race && (race.status === 'finished' || race.status === 'void') ? race.id : null
  ...
  if (race?.status === 'paddock') {
    return (
      <Attract>
        <FieldBoard race={race} bets={bets} title={ATTRACT.fieldTitle} subtitle={ATTRACT.paddockSub} />
      </Attract>
    )
  }
  ...
  if (race?.status === 'closed') {
    return (
      <Attract>
        <FieldBoard race={race} bets={bets} title={ATTRACT.closedTitle} subtitle={ATTRACT.closedSub} />
      </Attract>
    )
  }
