...
function RotatingLine() {
  ...
  useEffect(() => {
    const id = setInterval(() => setI((x) => (x + 1) % ATTRACT.lines.length), LINE_MS)
...
function PlayerCount({ players }: { players: readonly PlayerRow[] | undefined }) {
  if (!players) return null
  const last = players.reduce<PlayerRow | undefined>((a, p) => (!a || p.created_at > a.created_at ? p : a), undefined)
  return (
    <div className="flex flex-col gap-1">
      <p className="flex items-center gap-4 text-tv-sm text-ink">
        ...
        {players.length === 0 ? (
          ...
          <span key={players.length} className="inline-block origin-left animate-count-pop font-bold">
            {ATTRACT.players(players.length)}
          </span>
        )}
      </p>
      {last && <p className="truncate pl-8 text-2xl text-ink-dim">{ATTRACT.lastIn(playerLabel(last.name, last.tag))}</p>}
    </div>
  )
}
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="plate rounded-3xl bg-plate p-4 shadow-[0.6rem_0.6rem_0_var(--color-sleaze)]">
            <div className="unplate overflow-hidden rounded-xl">
              <QrCode value={joinUrl} label={ATTRACT.qrLabel(host)} className="size-[min(16rem,30dvh)]" />
            </div>
          </div>
          <p className="mt-2 font-display text-tv-md font-black text-plate uppercase">{ATTRACT.scan}</p>
          <p className="rounded-full bg-sleaze px-5 py-1.5 font-display text-tv-sm font-black text-white uppercase">
            {ATTRACT.bonus(fmtRm(WELCOME_BONUS))}
          </p>
