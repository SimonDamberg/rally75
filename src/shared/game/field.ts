...
import type { Field, HorsePublic, HorseStats, KuskInput, RaceCard } from './types'
import { ADJ, EFTERLED, EFTERNAMN, EPITET, FORLED, FORNAMN, KUSKTITEL, ORTER, SUBST } from '../content/names'
...
/**
 * Builds an n-horse field. `kusks` are the stable's own (active) kuskar; 2 to 4 of them
 * (capped by how many exist) get random slots, the rest are random kuskar.
 */
export function buildField(n: number, kusks: readonly KuskInput[], r: Rng): Field {
  const namedPool = r.shuffle(kusks)
  const namedCount = Math.min(
    namedPool.length,
    n,
    MIN_NAMED_KUSKAR + r.int(MAX_NAMED_KUSKAR - MIN_NAMED_KUSKAR + 1),
  )
  ...
  for (let i = 0; i < n; i++) {
    ...
    do {
      name = horseName(r)
      guard++
    } while ((usedNames.has(name) || usedTails.has(finalWord(name))) && guard < 80)
    usedNames.add(name)
    ...
    if (named) {
      jockey = named.name
      title = named.title
      jnote = named.notes.length ? r.pick(named.notes) : r.pick(KUSK_NOTER)(jockey, r)
    } else {
      guard = 0
      do {
        jockey = jockeyName(r)
  ...
  horses.forEach((h, i) => {
    ...
    h.baseOdds = round(Math.min(58, Math.max(1.45, (1 / p) * 0.86 * r.float(0.88, 1.16))), 2)
  })

  return { horses, stats }
}

export function buildRaceCard(kusks: readonly KuskInput[], r: Rng, n = FIELD_SIZE): RaceCard {
